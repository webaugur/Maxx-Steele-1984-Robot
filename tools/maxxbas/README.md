...
A USB copy lives in [`portable/MaxxSim`](../../portable/MaxxSim/README.txt). The Start scripts open the GUI. Window state is stored in `config/` next to those scripts, via `current_exe()`, not in the host profile. `sh portable/build_linux.sh` fills in `linux/maxx` and `hackrf/linux/` (`hackrf_info`, `hackrf_transfer`, `libhackrf`). The simulator looks there before `PATH`. Windows and macOS binaries are GitHub Actions artifacts.
